...
1. Sobre Computação Gráfica e Processamento de Imagens:
    ...
    ```
    Resposta: Criativo, com amplitude e atento a novas tecnologias para se aprimorar suas habilidades e tecnicas.
    ```
    
    b. Onde podemos aplicar ou encontrar aplicação de recursos da computação gráfica.
    ```
    Resposta: Manipulaçao de imagens, gerar modelagens 2D/3D, gerar objetos para animaçoes e jogos, mapas, leitor biometrico, reconhecimento facil e entre outros.
    ```
...
3. Como podemos definir e exemplificar a seguinte imagem abaixo:
   ...
   ![Q3_Fig1](/avaliacao_01/data/img1.jpg)
    ...
    ```
    Podemos manipular a imagens capturadas do mundo real ou podemos produzir novas imagens atraves de modelagens 2/3D.
    ```
